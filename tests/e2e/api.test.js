...
import portfinder from "pontfinder";
...
describe("API E2E Suite Test", () => {
  ...
  describe("static files", () => {
    test("GET /file.js - it should respond with 404 if file doesn't exists", async () => {
      const file = "file.js";
  ...
  describe("client workflow", () => {
    ...
    test("sending all commands at once together should not break the API", async () => {
      ...
      pipeAndReadStreamData(server.testServer.get("/stream"), onChunk);
      const commands = Reflect.ownKeys(possibleCommands).filter((cmd) => cmd !== possibleCommands.start || cmd !== possibleCommands.stop);
    ...
    test("it shouldn't break sending commands to the API if theres no audio playing", async () => {
      const server = await getTestServer();
      const sender = commandSender(server.testServer);
      await setTimeout(RETENTION_DATA_PERIOD);
      await sender.send(possibleCommands.stop);
