...
describe("#View - test suite for presentation layer", () => {
  ...
  function makeClassListElement({ classes } = { classes: [] }) {
  ...
  test("#onStartClicked", async () => {
    ...
    const text = "Start";
    const btn = makeBtnElement({ text });

    jest.spyOn(document, "querySelectorAll").mockReturnValueOnce([btn]);

    const eventOnClick = { srcElement: btn };
  ...
  test("#toggleDisableCommandBtn active=false should remove hidden class", () => {
    ...
    const classListWithoutActiveClass = makeClassListElement({ classes: [cssClass] });
    const view = new View();
    view.toggleDisableCommandBtn(classListWithoutActiveClass);
  ...
  test("#onStopBtn", async () => {
    ...
    const onClickBtnElement = {
      srcElement: makeBtnElement({ text }),
    };
  ...
  test("#toggleBtnStart given active = true it should add the hidden class to brnStart and remove hidden from btnStop", () => {
    const cssClass = "hidden";
    const btnStart = makeBtnElement({ classList: makeClassListElement() });
    const btnStop = makeBtnElement({
      classList: makeClassListElement({ classes: [cssClass] }),
    });
    const view = new View();
  ...
  test("#toggleBtnStart given active = false it should remove the hidden class from btnStart and add hidden from btnStop ", () => {
    ...
    const btnStop = makeBtnElement({ classList: makeClassListElement() });
    const view = new View();
    view.btnStart = btnStart;
    view.btnStop = btnStop;
  ...
  test("#btnIsActive - should be unassigned button if any the classes are part of ignoreButtons prop", () => {
    const view = new View();
    const cssUnassignedButton = [...view.ignoreButtons.values()];
    const btnStart = makeBtnElement({ classList: makeClassListElement({ classes: cssUnassignedButton }) });
    const result = view.notIsUnassignedButton(btnStart);

    expect(result).toBeFalsy();
  });

  test("#btnIsActive - should not be unassigned if none classes are part of ignoreButtons prop", () => {
    const view = new View();
    const cssClassToIgnoreButton = ["abc"];
    const btnStart = makeBtnElement({ classList: makeClassListElement({ classes: cssClassToIgnoreButton }) });
    const result = view.notIsUnassignedButton(btnStart);
  ...
  test("#View.constructor", () => {
    ...
    expect(view.btnStart).toStrictEqual(id)
    expect(view.btnStop).toStrictEqual(id)
