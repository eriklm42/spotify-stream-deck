...
describe("#Service - test suite for core processing", () => {
  ...
  test("#appendFxStream", async () => {
    ...
    jest.spyOn(service, service.mergeAudioStreams.name).mockReturnValue(mergedThrottleTransformMock);
  ...
  test("#readFxByName - it should return the song", async () => {
    ...
    jest.spyOn(fsPromises, fsPromises.readdir.name).mockResolvedValue([fxOnDisk]);
  ...
  test("#readFxByName - it should reject when song wasn't found", async () => {
    const service = new Service();
    const inputFx = "song01";
